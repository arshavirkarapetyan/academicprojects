...
def make_let_frame(bindings, env):
    ...
    check_formals(symb)
    result = Frame.make_child_frame(env, symb, val)
    return result
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    return MacroProcedure(expressions.first, expressions.second, env)
    # END Problem 21
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        check_procedure(procedure)
        result = Procedure.eval_call(procedure, expr.second, env)
        return result
        # END
