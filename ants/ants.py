...
class Bee(Insect):
    ...
    def __init__(self, armor): ###special handling for EC
        self.direction=True
    ...
    def blocked(self):
        ...
        if self.place.ant == None:
            return False
        elif self.place.ant.block_path == False:
            return False
...
# END Problem 13
    ...
    # BEGIN Problem 13
    implemented = True
    impostor=False # Change to True to view in the GUI
    # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while behind:
                if behind.ant:
                    if behind.ant not in QueenAnt.buffed:
                        QueenAnt.buffed.append(behind.ant)
                        behind.ant.damage *=2
                    if behind.ant in QueenAnt.buffed and behind.ant.is_container:
                        if behind.ant.contained_ant and behind.ant.contained_ant not in QueenAnt.buffed:
                                QueenAnt.buffed.append(behind.ant.contained_ant)
...
def apply_effect(effect, bee, duration):
    ...
    i=1
    original =bee.action
    effected=effect(original, bee)
    def helper(colony):
        nonlocal i
        if i<=duration:
            i+=1
            return effected(colony)
        else:
            original(colony)
    bee.action=helper
